...
<details>
  <summary><strong>Вопрос: Мои локальные тесты (`pytest`) постоянно падали. Что было не так?</strong></summary>
  
  > **Ответ:** Да, в процессе отладки тестов мы столкнулись с целой серией классических проблем. Ваш лог идеально отражает этот путь. Вот эти проблемы и их финальные решения:
  >
  > 1.  **`assert 200 == 303` или `KeyError: 'location'`**: Эта ошибка возникала, потому что наш эндпоинт после успешного создания/удаления делает редирект (возвращает статус `303 See Other` и заголовок `Location: /`). Однако `TestClient` в `pytest` по умолчанию **автоматически следует за редиректами**. Поэтому тест получал уже финальную страницу со статусом `200 OK`, в которой, естественно, не было заголовка `location`.
  >     -   **Решение:** Мы изменили тесты так, чтобы они проверяли `status_code == 200` и анализировали HTML-код **итоговой страницы**, а не пытались перехватить редирект.
  >
  > 2.  **`AssertionError: 'Reminder for Tomorrow' in '<!DOCTYPE...>'`**: Тесты падали, потому что созданные напоминания не появлялись на странице.
  >     -   **Причина:** В одной из версий тестов мы пытались создать данные через **API-эндпоинт** (`/api/reminders/`), который был предназначен для межсервисного взаимодействия, а не через **эндпоинт для HTML-форм** (`/reminders/`). Они работали по-разному.
  >     -   **Решение:** Мы исправили все тесты так, чтобы они имитировали поведение пользователя и отправляли POST-запросы именно на те эндпоинты, которые обрабатывают формы, с правильными именами полей (например, `start_time_str`, а не `start_time`).
  >
  > 3.  **`AssertionError: Не удалось найти ID созданного напоминания`**: Тест падал, потому что не мог найти ID для удаления.
  >     -   **Решение:** Вместо того, чтобы жестко прописывать `id=1`, мы добавили в тест поиск ID с помощью регулярного выражения `re.search(r'ID: (\d+)', response.text)`. Это гарантирует, что мы всегда удаляем именно тот элемент, который только что создали.
...
-   **Resource Requests**: Чтобы HPA мог вычислить утилизацию CPU (например, `15%`), он должен знать, сколько ресурсов Под "забронировал" при старте. Мы уже указали это в нашем манифесте `k8s/backend.yaml`, добавив блок `resources`:
    ...
    ...
      spec:
        ...
        - name: backend
          ...
          # Этот блок критически важен для HPA!
          resources:
            requests:
...
1.  **В первом терминале** запустим наблюдение за состоянием HPA. Он будет показывать текущую и целевую нагрузку, а также количество реплик.
    ```bash
    kubectl get hpa -w
